/* ═══════════════════════════════════════════════════════════════════════
   PROBLEM LOG — write-ups of things that broke and what fixed them.
   This is the file you edit to "post" a new entry.

   ⚠️  READ BEFORE PUBLISHING
   The three entries below were seeded from build logs and screenshots found
   in your Downloads folder — the symptoms are real and quoted verbatim, but
   the root causes and fixes are the *most likely* explanation, not a record
   of what you actually did. Rewrite each one to match what really happened,
   or delete it. Nothing here should go public unverified.

   ── Schema ────────────────────────────────────────────────────────────
   slug        string    URL fragment — #log/<slug> deep-links to the entry
...
export const posts = [
  {
    ...
    symptom:
      'The reporting service failed to start in UAT. Locally it ran fine — the difference only showed up in the packaged jar.',
    evidence: {
      label: 'kubectl logs reporting-service-uat',
...
Failed to bind properties under 'spring.profiles.active' to java.util.Set<java.lang.String>:
    ...
    fix: [
      ...
      'Unpacked the built jar in CI and asserted the property no longer contains an @ — the check that would have caught this before the deploy.',
    ],
    code: {
      label: 'pom.xml',
      body: `<build>
...

  {
    ...
    evidence: {
      ...
      body: `[ERROR] Failed to execute goal
        ...
        (repackage) on project user-service-be:
...
Total time:  01:26 min`,
...

  {
    ...
    cause:
      'Inbound messages were already deduplicated by message ID before hitting Kafka. Outbound ones were not. The expiry sweep had no idempotency key, so any second trigger — a retry, or another instance running the same sweep — produced a second send.',
    fix: [
...
cloudApi.send(conversationId, message);`,
    ...
    takeaway:
      'Inbound dedup is the obvious half. Anything that can fire twice — a retry, a scheduler on two pods, a rebalance — needs an idempotency key on the way out too, and a metric so you can prove it is working.',
  },
]
